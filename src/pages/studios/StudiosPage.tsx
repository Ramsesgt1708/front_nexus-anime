...
import AddStudiosForm from "./addStudiosForm";
...
function StudiosPage() {
  ...
  const deleteAnime = async (id: number, nombre: string) => {
    const result = await SweetWarning.fire({
      title: "¿Estás seguro?",
      html: `Vas a eliminar el género <strong>"${nombre}"</strong>. <br>Esta acción es irreversible.`,
    ...
    if (result.isConfirmed) {
      try {
        await clientAPI.delete(`/Estudios/${id}`);
  ...
  const toggleStatus = async (id: number, nombre: string) => {
    try {
      await clientAPI.patch(`/Estudios/${id}/toggle-status`);
  ...
  const columns = [
      ...
      cell: (row: any) => (
        <div className="flex flex-col text-xs text-slate-400">
          <span className="text-slate-300 text-xs whitespace-nowrap">
            {formatDateLocal(row.fechaCreacion)}
          </span>
        </div>
      ),
    },
    {
      name: "Modificado",
      sortable: true,
      selector: (row: any) => row.fechaModificacion,
      cell: (row: any) => (
        <div className="flex flex-col text-xs text-slate-400">
          <span className="text-slate-300 text-xs whitespace-nowrap">
            {formatDateLocal(row.fechaModificacion)}
          </span>
        </div>
      ),
    },
      ...
      cell: (row: any) => {
        const actions: ActionMenuItem[] = [
          ...
          },
          {
            ...
            onClick: () => {
              setOpenMenuId(null);
              deleteAnime(row._id, row.nombre);
            },
            color: "text-[#ffc7c7]",
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <div className="border-b border-slate-800 pb-6">
          ...
          <p className="mt-2 text-slate-400">
            Gestiona las etiquetas y categorías principales para la
            clasificación del contenido en la plataforma.
          </p>
        </div>
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Los estudios son entidades que representan las compañías o
              grupos responsables de la producción y distribución del contenido
              audiovisual. Al gestionar los estudios, aseguras una correcta
              atribución y clasificación del contenido en la plataforma.
            </p>
          </div>
        </div>
          ...
          <div className="flex w-full md:w-auto gap-4">
            ...
            <div className="bg-slate-900 border border-slate-800 px-5 py-3 rounded-xl flex items-center gap-4 min-w-[140px]">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-white">{rows.filter(row => row.isActive).length}</p>
              </div>
            </div>
            <div className="bg-slate-900 border border-slate-800 px-5 py-3 rounded-xl flex items-center gap-4 min-w-[140px]">
