...
interface AddStudiosProps {
  show: boolean;
  onRequestClose: (refresh: boolean) => void;
  payload?: any;
}
export default function AddStudiosForm({
  show,
  onRequestClose,
  payload,
}: AddStudiosProps) {
  ...
  const onSubmit = async (data: typeof defaultValues) => {
    ...
    try {
      if (payload) {
        await clientAPI.put(`/Estudios/${payload._id}`, data);
      } else {
        await clientAPI.post("/Estudios", data);
      }
      reset(defaultValues);
      onRequestClose(true);
      ...
      if (errores && errores.length > 0) {
        ...
        toast.error(
          error?.response?.data?.message || "Error al guardar el estudio."
        );
      }
    }
  };
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-2xl w-full max-w-md">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
            <h2 className="text-xl font-bold text-white">
              {payload?._id ? "Editar" : "Agregar"} Estudio
            </h2>
            <button
              onClick={handleClose}
            ...
            <div className="px-6 py-6">
              <div className="mb-4">
                <Input
                  ...
                  placeholder="Ej: Studio Ghibli, MAPPA, Wit Studio..."
