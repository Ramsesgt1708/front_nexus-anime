...
import AddPlanesForm from "./addPlanesForm";
...
function PlanesPage() {
  ...
  const deletePlan = async (id: string, nombre: string) => {
    const result = await SweetWarning.fire({
      title: "¿Estás seguro?",
      html: `Vas a eliminar el plan <strong>"${nombre}"</strong>. <br>Esta acción es irreversible.`,
  ...
  const toggleStatus = async (id: string, nombre: string) => {
    try {
      await planesService.toggleStatus(id);
  ...
  const getPlanes = async () => {
    try {
      const response = await planesService.getAll();
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Los planes registrados aquí serán ofrecidos a los usuarios para
              que se suscriban. Asegúrese de definir correctamente el nombre,
              descripción y precio de cada plan.
            </p>
          </div>
        </div>
