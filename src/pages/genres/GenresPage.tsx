...
import AddGenresForm from "./addGenresForm";
...
function GenresPage() {
  ...
  const deleteAnime = async (id: number, nombre: string) => {
    ...
    if (result.isConfirmed) {
      try {
        await clientAPI.delete(`/Generos/${id}`);
  ...
  const toggleStatus = async (id: number, nombre: string) => {
    try {
      await clientAPI.patch(`/Generos/${id}/toggle-status`);
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Los géneros registrados aquí se utilizarán para alimentar los
              filtros de búsqueda en la aplicación de usuario y para generar
              reportes estadísticos. Asegúrese de no duplicar categorías (ej.
              "Accion" y "Acción").
            </p>
          </div>
        </div>
