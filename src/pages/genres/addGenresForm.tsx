...
interface AddGenresProps {
  show: boolean;
  onRequestClose: (refresh: boolean) => void;
  payload?: any;
}
export default function AddGenresForm({
  show,
  onRequestClose,
  payload,
}: AddGenresProps) {
  ...
  const onSubmit = async (data: typeof defaultValues) => {
    ...
    try {
      if (payload) {
        await clientAPI.put(`/Generos/${payload._id}`, data);
      } else {
        await clientAPI.post("/Generos", data);
      }
      reset(defaultValues);
      onRequestClose(true);
      ...
      if (errores && errores.length > 0) {
        ...
        toast.error(
          error?.response?.data?.message || "Error al guardar el género."
        );
      }
    }
  };
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-lg shadow-2xl w-full max-w-md">
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700">
            <h2 className="text-xl font-bold text-white">
              {payload?._id ? "Editar" : "Agregar"} Género
            </h2>
            <button
              onClick={handleClose}
