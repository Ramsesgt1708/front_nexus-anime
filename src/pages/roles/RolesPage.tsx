...
function RolesPage() {
  ...
  const deleteRole = async (id: string, nombre: string) => {
    const result = await SweetWarning.fire({
      title: "¿Estás seguro?",
      html: `Vas a eliminar el rol <strong>"${nombre}"</strong>. <br>Esta acción es irreversible.`,
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <div className="border-b border-slate-800 pb-6">
          ...
          <p className="mt-2 text-slate-400">
            Gestiona los roles y permisos de usuario en la plataforma.
          </p>
        </div>
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex gap-4 items-start">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              Los roles definidos aquí determinan los permisos y acceso que
              tendrán los usuarios en la plataforma. Asegúrese de crear roles
              únicos y bien definidos para mantener la seguridad del sistema.
            </p>
          </div>
        </div>
